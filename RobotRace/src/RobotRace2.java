...
public class RobotRace2 extends Base {
    ...
    public void setView() {
        ...
        gl.glFrustum(-1.0, 1.0, -1.0, 1.0, 4.0, 20.0);

        // Set camera.
        gl.glMatrixMode(GL_MODELVIEW);
        gl.glLoadIdentity();

        glu.gluLookAt((gs.vDist - gs.cnt.x()) * Math.cos(gs.phi) * Math.cos(gs.theta),
                      (gs.vDist - gs.cnt.x()) * Math.sin(gs.phi) * Math.cos(gs.theta),
                      Math.sin(gs.theta), // eye point
    ...
    public void drawAxisFrame() {
        ...
        if (gs.showAxes) {
            ...
            gl.glRotatef(90, 0, 1, 0);
            glut.glutSolidCone(0.05f, 0.10f, 10, 10);
            gl.glPopMatrix();
